...
function PreLoadedReader(data, text) {
	this.data = data;
	this.text = text;
}

PreLoadedReader.prototype = {
		evts: {},
		...
		readAsArrayBuffer: function( file ) {
			if(this.evts['load']) {
				var buffer = new ArrayBuffer(this.data.length);
...
var Loader = function ( editor ) {
	...
	this.loadFile = function ( file, preloaded, cb ) {
		...
		if(!preloaded) {
			...
			reader = new PreLoadedReader(file.data, file.text);
		}
		...
		switch ( extension ) {
			...
			case 'opm':
				...
				reader.addEventListener( 'load', function ( event ) {
					...
					var object = new THREE.OPMLoader().parse( contents );
					object.name = filename;
